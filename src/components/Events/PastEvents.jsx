...
const pastEvent = {
  ...
  description:
    "From powerful talks to unforgettable moments, TEDxAlgeria’s past events have sparked change, and inspired thousands. Take a look back at the highlights, speakers, and moments that shaped our journey.",
  date: "April 6, 2024",
...
const PastEvents = () => {
  return (
    <section className="relative bg-white py-20 overflow-hidden">
      {/* Decorative X with subtle hover animation */}
      <div className="absolute inset-0 z-0 hidden md:block">
        <motion.div
          ...
          className="absolute top-60 left-75 w-1/3 h-full bg-no-repeat bg-left bg-contain"
      ...
      <div className="container mx-auto px-4 relative z-20 flex flex-col-reverse md:flex-row">
        ...
        <div className="w-full md:w-1/2 p-4">
          ...
          >
            <Link
              to={`/events/${pastEvent.id}`}
              className="group block relative overflow-hidden rounded-lg shadow-sm hover:shadow-xl transition-shadow duration-500 w-full md:w-xl md:ml-20"
            >
              <motion.img
                src={pastEvent.image}
                alt={pastEvent.title}
              ...
              />
              <motion.div
                className="absolute inset-0 bg-gradient-to-t from-black via-black to-transparent opacity-70 group-hover:opacity-90 flex flex-col justify-end"
                transition={{ duration: 0.5 }}
              >
                <motion.div
                  className="relative z-10 text-white p-4"
                  initial={{ y: 10, opacity: 0.8 }}
                  whileHover={{ y: 0, opacity: 1 }} // Subtle slide-up on hover
            ...
            <div className="flex justify-end">
              <Link to={`/events/${pastEvent.id}`}>
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="px-8 py-2 rounded-full bg-white text-black border-2 border-[#F4A261] font-semibold shadow-md hover:bg-[#F4A261] hover:text-white transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-[#F4A261] focus:ring-offset-2 cursor-pointer"
